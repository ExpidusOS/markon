...
static gboolean
markon_looks_like_an_uri (const gchar *string)
{
...
GFile *
_markon_file_new_for_unknown_input (const gchar *path,
                                    GFile       *parent)
{
  ...
  if (markon_looks_like_an_uri (path))
...
GFile *
_markon_file_new_relative_to_file (const gchar *path,
                                   GFile       *file)
{
  GFileType type;
  GFile    *result;
  ...
  result = _markon_file_new_for_unknown_input (path, dir);
  g_object_unref (dir);
...
gchar *
_markon_file_get_uri_relative_to_file (const gchar *path,
                                       GFile       *file)
{
  GFile *absolute_file;
  gchar *uri;

  absolute_file = _markon_file_new_relative_to_file (path, file);
