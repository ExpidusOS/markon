            ...
            const gchar *path)
{
  ...
  if (!markon_menu_element_get_visible (MARKON_MENU_ELEMENT (menu)))
    return;

  /* Determine menu name */
  directory = markon_menu_get_directory (menu);
  ...
  else
    {
      name = g_strdup_printf ("%s%s/", path,
                              (directory == NULL
                               ...
                               : markon_menu_directory_get_name (directory)));
    }
  ...
  for (iter = menus; iter != NULL; iter = g_list_next (iter))
    {
      ...
      if (G_LIKELY (markon_menu_element_get_visible (iter->data)))
        print_menu (MARKON_MENU (iter->data), name);
    }
  ...
  for (iter = items; iter != NULL; iter = g_list_next (iter))
    {
      if (MARKON_IS_MENU_ITEM (iter->data)
          && markon_menu_element_get_visible (iter->data))
        {
          file = markon_menu_item_get_file (iter->data);
          file_path = g_file_get_path (file);

          g_printf ("%s\t%s\t%s\n", name, markon_menu_item_get_desktop_id (iter->data),
                    file_path);
      ...
      char **argv)
{
  ...
  if (G_LIKELY (markon_menu_load (menu, NULL, &error)))
    {
  ...
  else
    {
      ...
      uri = g_file_get_uri (markon_menu_get_file (menu));
